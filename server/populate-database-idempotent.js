...
async function createAuthorsIfNeeded() {
    ...
    for (const authorInfo of authorData) {
        ...
        if (!existingAuthor) {
            ...
            if (error) {
                console.error(`Error creating author ${authorInfo.name}:`, error);
                continue;
            }
...
async function createBooksIfNeeded(token, authorIdMap) {
    ...
    for (const bookData of booksData) {
        ...
        if (!existingBook) {
            ...
            const authorId = authorIdMap[bookData.authorName];
            if (authorId) {
                await supabase
                    .from('book_authors')
                    .insert({
            ...
            console.log(`Created new book: ${bookData.title}`);
...
async function createReservationsIfNeeded(books, users) {
    ...
    const reservationPairs = [
        { bookIndex: 0, userIndex: 1 },
        { bookIndex: 2, userIndex: 3 },
        { bookIndex: 4, userIndex: 0 }
    ];
